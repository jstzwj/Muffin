...
int main(int argc, char** argv) {
  ...
  require(argc == 2, QStringLiteral("Expected Info pixel manifest"));
  const QString manifestPath = QString::fromLocal8Bit(argv[1]);
  QFile file(manifestPath);
  require(file.open(QIODevice::ReadOnly), file.errorString());
  const QJsonObject root = QJsonDocument::fromJson(file.readAll()).object();
  require(root.value(QStringLiteral("fixtureSha256")).toString() ==
              QLatin1String("c5d8b87f2466925e155b781c49d839354f1cda9e456ebb748b3279a775f1e5d7"),
          QStringLiteral("Info pixel fixture changed"));
  const QJsonArray cases = root.value(QStringLiteral("cases")).toArray();
  require(cases.size() == 2, QStringLiteral("Expected two Info pixel cases"));
