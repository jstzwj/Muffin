...
qreal textAdvance(const GitGraphSceneStyle& style, const QString& text,
                  qreal size, bool bold = false) {
  const editor::CssPixelFont css = font(style, size, bold);
  qreal width = css.horizontalAdvance(text);
  if (const auto hb = textmetrics::harfBuzzAdvance(
          text, style.fontFamily, size))
    width = std::ceil(*hb * 64.0) / 64.0;
  return width;
}

QRectF directTextBounds(const GitGraphSceneStyle& style, const QString& text,
                        qreal size, bool bold = false,
                        bool expandBoldRight = true) {
  const editor::CssPixelFont css = font(style, size, bold);
  ...
  // same fringe before using the box for getBBox-driven layout.
  const qreal skiaLeft = inkLeft >= 0.0 && inkLeft < size * 0.02 ? -1.0 : inkLeft;
  qreal left = std::min(0.0, skiaLeft);
  qreal right = std::max(textAdvance(style, text, size, bold), inkRight);
  if (bold) {
    left -= 1.0;
    if (expandBoldRight) right += 1.0;
  }
  return QRectF(left, -std::ceil(1.05 * size), right - left,
                std::ceil(1.35 * size));
}
...
QRectF branchTextBounds(const GitGraphSceneStyle& style,
                        const QStringList& lines, bool bold = false) {
  qreal left = 0.0;
  qreal right = 0.0;
  for (const QString& line : lines) {
    const QRectF horizontal = directTextBounds(style, line, style.fontSize, bold);
  ...
  return QRectF(left, -1.0, right - left,
                std::ceil(style.fontSize * 1.35) +
                    style.fontSize * std::max<qsizetype>(0, lines.size() - 1));
}

QRectF branchTextBounds(const GitGraphSceneStyle& style, const QString& text) {
  return branchTextBounds(style, branchTextLines(text));
}
...
GitGraphScene buildGitGraphScene(const GitGraphData& data,
  ...
  for (qsizetype i = 0; i < data.orderedBranches.size(); ++i) {
    ...
    const QRectF label = branchTextBounds(scene.style, name);
    branchPos += 50.0 + (config.rotateCommitLabel ? 40.0 : 0.0) +
                 (data.direction == Direction::LeftToRight ? 0.0 : label.width() / 2.0);
  }
  ...
  auto append = [&](GitGraphPrimitive primitive) {
    unite(content, hasBounds, primitive.bounds); scene.primitives.push_back(std::move(primitive));
  };
  ...
  if (config.showBranches) {
    for (qsizetype i = 0; i < data.orderedBranches.size(); ++i) {
      ...
      line.bounds = QRectF(line.line.p1(), line.line.p2()).normalized(); append(line); layout.lanes.push_back(spine);

      const QStringList branchLines = branchTextLines(name);
      const QRectF tb = branchTextBounds(scene.style, branchLines, redux);
      ...
      bkg.bounds = transformedBounds(bkg.rect, bkg.translation); append(bkg);
      ...
      label.bounds = transformedBounds(tb, label.translation); append(label);
    }
  }
  ...
  for (const GitCommit* commitPointer : ordered) {
    ...
    if (symbol == CommitType::Highlight) {
      GitGraphPrimitive outer; outer.kind=PrimitiveKind::Rect; outer.role=QStringLiteral("commit-highlight-outer"); outer.rect=QRectF(p.x()-(redux?7:10),p.y()-(redux?7:10),redux?14:20,redux?14:20); outer.bounds=outer.rect;
      ...
      outer.stroke=colorTheme&&ci==0?scene.style.nodeBorder:outer.fill; append(outer);
      GitGraphPrimitive inner=outer; inner.role=QStringLiteral("commit-highlight-inner"); inner.rect=QRectF(p.x()-(redux?4:6),p.y()-(redux?4:6),redux?8:12,redux?8:12); inner.bounds=inner.rect; inner.fill=isNeoTheme(scene.style.themeName)?scene.style.mainBkg:scene.style.primaryColor; inner.stroke=inner.fill; append(inner);
    } else {
      GitGraphPrimitive bullet; bullet.kind=PrimitiveKind::Circle; bullet.role=QStringLiteral("commit"); bullet.center=p; bullet.radius=redux?7:10; bullet.bounds=QRectF(p.x()-bullet.radius,p.y()-bullet.radius,2*bullet.radius,2*bullet.radius); bullet.fill=symbol==CommitType::CherryPick?(isNeoTheme(scene.style.themeName)?scene.style.nodeBorder:scene.style.textColor):baseColor; bullet.stroke=symbol==CommitType::CherryPick?QStringLiteral("none"):baseColor; append(bullet);
      if (symbol==CommitType::Merge) { GitGraphPrimitive inner=bullet; inner.role=QStringLiteral("commit-merge"); inner.radius=redux?5:6; inner.bounds=QRectF(p.x()-inner.radius,p.y()-inner.radius,2*inner.radius,2*inner.radius); inner.fill=isNeoTheme(scene.style.themeName)?scene.style.mainBkg:scene.style.primaryColor; inner.stroke=inner.fill; append(inner); }
      if (symbol==CommitType::Reverse) { GitGraphPrimitive cross; cross.kind=PrimitiveKind::Path; cross.role=QStringLiteral("commit-reverse"); const qreal s=redux?4:5; cross.path.moveTo(p.x()-s,p.y()-s);cross.path.lineTo(p.x()+s,p.y()+s);cross.path.moveTo(p.x()-s,p.y()+s);cross.path.lineTo(p.x()+s,p.y()-s);cross.pathData=QStringLiteral("M %1,%2L%3,%4M%1,%4L%3,%2").arg(number(p.x()-s),number(p.y()-s),number(p.x()+s),number(p.y()+s));cross.bounds=cross.path.boundingRect();cross.stroke=isNeoTheme(scene.style.themeName)?scene.style.mainBkg:scene.style.primaryColor;cross.strokeWidth=isNeoTheme(scene.style.themeName)?scene.style.strokeWidth:3;append(cross); }
      if (symbol==CommitType::CherryPick) { const QString eye=(scene.style.themeName.contains(QLatin1String("dark"))?QStringLiteral("#000000"):QStringLiteral("#fff")); for(qreal dx:{-3.0,3.0}){GitGraphPrimitive dot;dot.kind=PrimitiveKind::Circle;dot.role=QStringLiteral("cherry-dot");dot.center=QPointF(p.x()+dx,p.y()+2);dot.radius=redux?2.5:2.75;dot.bounds=QRectF(dot.center.x()-dot.radius,dot.center.y()-dot.radius,2*dot.radius,2*dot.radius);dot.fill=eye;dot.stroke=QStringLiteral("none");append(dot);} for(qreal dx:{3.0,-3.0}){GitGraphPrimitive stem;stem.kind=PrimitiveKind::Line;stem.role=QStringLiteral("cherry-stem");stem.line=QLineF(p.x()+dx,p.y()+1,p.x(),p.y()-5);stem.bounds=QRectF(stem.line.p1(),stem.line.p2()).normalized();stem.stroke=eye;append(stem);} }
    }
    if (config.showCommitLabel && commit.type != CommitType::CherryPick && (commit.type != CommitType::Merge || commit.customId)) {
      const QRectF tb=directTextBounds(scene.style,commit.id,scene.style.commitLabelFontSize,redux,false);
      GitGraphPrimitive bg;bg.kind=PrimitiveKind::Rect;bg.role=QStringLiteral("commit-label-background");bg.fill=isNeoTheme(scene.style.themeName)?QStringLiteral("transparent"):scene.style.commitLabelBackground;bg.opacity=isNeoTheme(scene.style.themeName)?1.0:0.5;
      GitGraphPrimitive label;label.kind=PrimitiveKind::Text;label.role=QStringLiteral("commit-label");label.text=commit.id;label.fontSize=scene.style.commitLabelFontSize;label.bold=redux;label.fill=isNeoTheme(scene.style.themeName)?scene.style.nodeBorder:scene.style.commitLabelColor;
      if(data.direction==Direction::LeftToRight){
      ...
      append(bg);append(label);
    }
    if(!commit.tags.isEmpty()) {
      qreal maxW=0,maxH=0;for(const QString&t:commit.tags){const QRectF tb=directTextBounds(scene.style,t,scene.style.tagLabelFontSize);maxW=std::max(maxW,tb.width());maxH=std::max(maxH,tb.height());}
      for(qsizetype ti=0;ti<commit.tags.size();++ti){
        const QString tag=commit.tags.at(commit.tags.size()-1-ti);
        const QRectF tb=directTextBounds(scene.style,tag,scene.style.tagLabelFontSize);
        ...
        GitGraphPrimitive poly;poly.kind=PrimitiveKind::Polygon;poly.role=QStringLiteral("tag-background");poly.fill=isNeoTheme(scene.style.themeName)?scene.style.mainBkg:scene.style.tagLabelBackground;poly.stroke=isNeoTheme(scene.style.themeName)?scene.style.nodeBorder:scene.style.tagLabelBorder;
        GitGraphPrimitive hole;hole.kind=PrimitiveKind::Circle;hole.role=QStringLiteral("tag-hole");hole.radius=1.5;hole.fill=scene.style.textColor;
        GitGraphPrimitive tx;tx.kind=PrimitiveKind::Text;tx.role=QStringLiteral("tag-label");tx.text=tag;tx.fontSize=scene.style.tagLabelFontSize;tx.fill=scene.style.tagLabelColor;
        if(data.direction==Direction::LeftToRight){
        ...
        poly.bounds=transformedBounds(poly.polygon.boundingRect(),poly.translation,poly.rotation,poly.rotationOrigin);append(poly);
        hole.bounds=transformedBounds(QRectF(hole.center.x()-1.5,hole.center.y()-1.5,3,3),hole.translation,hole.rotation,hole.rotationOrigin);append(hole);
        tx.bounds=transformedBounds(tb.translated(tx.position),tx.translation,tx.rotation,tx.rotationOrigin);append(tx);
      }
    }
  }
  if(!data.title.isEmpty()){QRectF tb=directTextBounds(scene.style,data.title,18);tb.setTop(-19.0);tb.setHeight(24.0);GitGraphPrimitive title;title.kind=PrimitiveKind::Text;title.role=QStringLiteral("title");title.text=data.title;title.fontSize=18;title.fill=scene.style.textColor;title.anchor=QStringLiteral("middle");const qreal center=hasBounds?content.center().x():0;title.position=QPointF(center,-config.titleTopMargin);title.bounds=tb.translated(center-tb.width()/2,-config.titleTopMargin);append(title);}
