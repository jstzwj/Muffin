...
| 4（cascade ✅ + dagre 75%） | ✅ | **style cascade 全部完成**：`MermaidStyleResolve` family-agnostic 模块（acafb05）+ class 边（b6c8480）+ state transition（53bb455）+ **ER entity**（5ee3c21）linkStyle/classDef/cssClass/style 接入。**所有 4 个有 classDef 语法的图（flowchart/class/state/er）均已接入 cascade**；sequence 无 classDef 语法（正确地走 theme/config，非 cascade 缺口）。dagre input ~75% 统一（flowchart/state/er/class-compound 共享 `layoutFlowchartNodesDagre`）。**仅剩 dagre gap**：class flat 分支重路由（行为变更）+ 投影 helper 抽取（边际 DRY）。 |
