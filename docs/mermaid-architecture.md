...
**2026-08-10 现状**：九个生产图族均通过单一 scene 指针和 `Diagram` registry
进入族无关的 editor/PNG/SVG/canvas/interaction 路径；九个 adapter 分离在各自
TU。新增的 Pie、Quadrant 和 Journey 均有真实 Mermaid 11.16.0 语法、几何和像素
oracle。完整 Release 门禁为 199/199。配置矩阵现为 178 行（91 parity /
8 partial / 7 unsupported / 44 upstream-inert / 5 deferred /
