...
| **配置** | 每个 config key 的效果与上游一致 | config-effect-matrix（359 行，逐 key 标 parity/partial/deferred） |
...
**2026-08-12 现状**：二十九个生产图族均通过单一 scene 指针和 `Diagram` registry
进入族无关的 editor/PNG/SVG/canvas/interaction 路径；二十九个 adapter 分离在各自
...
oracle。完整 Release 门禁为 280/280。配置矩阵现为 359 行（214 parity /
8 partial / 7 unsupported / 102 upstream-inert / 5 deferred /
