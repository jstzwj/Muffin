...
| **配置** | 每个 config key 的效果与上游一致 | config-effect-matrix（279 行，逐 key 标 parity/partial/deferred） |
...
oracle。完整 Release 门禁为 243/243。配置矩阵现为 279 行（159 parity /
8 partial / 7 unsupported / 77 upstream-inert / 5 deferred /
