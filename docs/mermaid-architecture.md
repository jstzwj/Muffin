...
| **配置** | 每个 config key 的效果与上游一致 | config-effect-matrix（298 行，逐 key 标 parity/partial/deferred） |
...
oracle。完整 Release 门禁为 252/252。配置矩阵现为 298 行（176 parity /
8 partial / 7 unsupported / 79 upstream-inert / 5 deferred /
