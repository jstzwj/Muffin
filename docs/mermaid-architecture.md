...
| 4（Phase 1 + class pilot） | ✅ | **MermaidStyleResolve** family-agnostic cascade module（acafb05）+ **class 边 linkStyle/edge classDef 接入**（b6c8480：ClassDiagramData 暴露 classDef 表 + 解析 `linkStyle <index>` + buildClassScene 解析边 + painter 用解析值，class pattern 保留、linkStyle 覆盖）。dagre input ~75% 已统一。**Phase 2 后续**：state transitions / sequence / er 接入（各需加 classDef 解析，同 feature-add 范式）+ dagre 剩余 gap（class flat 分支重路由、投影 helper）。 |
