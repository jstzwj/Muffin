...
| 4（Phase 1 + class/state pilot） | ✅ | **MermaidStyleResolve** family-agnostic cascade module（acafb05）+ **class 边**（b6c8480）+ **state transition**（53bb455）linkStyle/edge classDef 接入（解析 `linkStyle <index>` + scene 解析边 + painter 用解析值）。dagre input ~75% 已统一。**Phase 2 后续**：sequence / er 接入（需先加 classDef 解析）+ dagre 剩余 gap（class flat 分支重路由、投影 helper）。 |
