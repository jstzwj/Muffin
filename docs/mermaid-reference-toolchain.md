...
- **Real-mermaid geometry oracles** — fail-on-divergence tests that assert
  ...
  twenty-one native families are covered: flowchart (dagre-snapshots + geometry), ER
  ...
  Mindmap, TreeView, Event Modeling, Ishikawa, Venn, Gantt, and Info (family-specific geometry and pixel
  fixtures).
- `config-effect-matrix.json` covers all twenty-one native families.
