...
void DefaultTeXFontParser::setupFontInfo(__BasicInfo& bi, FontInfo& fi) {
  ...
  int metricIndex = 0;
  for (const __Metrics& m : bi.metrics) {
    const int i = metricIndex++;
    const size_t r = i * 5;
    metrics[r + 0] = m.ch;
    metrics[r + 1] = m.width;
  ...
  for (const __Extension& e : bi.extensions) {
    const int i = extensionIndex++;
    const size_t r = i * 5;
    exts[r + 0]    = e.ch;
    exts[r + 1]    = e.top;
    exts[r + 2]    = e.mid;
  ...
  int largerIndex = 0;
  for (const __Larger& l : bi.largers) {
    const int i = largerIndex++;
    const size_t r = i * 3;
    largers[r + 0] = l.code;
  ...
  int ligatureIndex = 0;
  for (const __Lig& l : bi.ligs) {
    const int i = ligatureIndex++;
    const size_t r  = i * 3;
    ligtures[r + 0] = l.left;
  ...
  int kernIndex = 0;
  for (const __Kern& k : bi.kerns) {
    const int i = kernIndex++;
    const size_t r = i * 3;
    kerns[r + 0]   = k.left;
    kerns[r + 1]   = k.right;
    kerns[r + 2]   = k.kern;
