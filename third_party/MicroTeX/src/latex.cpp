...
TeXRender* LaTeX::parse(const wstring& latex, int width, float textSize, float lineSpace, color fg) {
  return parse(latex.c_str(), latex.size(), width, textSize, lineSpace, fg);
}

TeXRender* LaTeX::parse(const wchar_t* latexData, size_t latexLength, int width, float textSize, float lineSpace, color fg) {
  wstring latex(latexData == nullptr ? L"" : latexData, latexLength);
  bool lined = true;
  if ((latex.size() >= 2 && latex[0] == L'$' && latex[1] == L'$')
      || (latex.size() >= 2 && latex[0] == L'\\' && latex[1] == L'[')) {
    lined = false;
  }
  ...
  TeXRender* render =
    builder.setStyle(TexStyle::display)
