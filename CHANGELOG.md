...
- **Unreadable selected rows in themed dialogs and sidebars** - A theme's selection tint is authored for prose text selection and may be translucent (newsprint: `rgba(32,43,51,.63)`); used directly as a chrome fill it composited against whatever sat underneath — inside the Windows 11 combo popup that band is unpainted black, so the selected dropdown item rendered black-on-black, and the active preferences tab was a washed-out gray with dark ink. Chrome surfaces now use a derived opaque pair: the tint flattened onto the surface, plus the best-contrasting text color (for newsprint, the white the theme itself ships for this tint) — across the combo popup, the preferences sidebar, and the file/outline trees
