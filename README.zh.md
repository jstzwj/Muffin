...
- **原生 Mermaid 图表** — 通过纯 C++/Qt 引擎实时渲染 ```` ```mermaid ```` 代码围栏中的流程图、时序图、类图和状态图，无 JavaScript 或浏览器运行时依赖。无效源码会保持可编辑，并显示精确的行列诊断、源码错误标记和点击跳转定位。共享标题/可访问性元数据、圆角流程、受控链接与提示、实时边动画、Sequence 链接菜单、确定性的原生 SVG/HTML 导出以及自动生成的 102 行配置生效矩阵明确约束编辑器和导出行为。
