...
- **原生 Mermaid 图表** — 通过纯 C++/Qt 引擎实时渲染 ```` ```mermaid ```` 代码围栏中的流程图、时序图、类图和状态图，无 JavaScript 或浏览器运行时依赖。无效源码会保持可编辑，并显示精确的行列诊断、源码错误标记和点击跳转定位。超大 scene 会跳过 dirty viewport 外的绘制工作，打印和导出仍保留完整图表。
...
- [ ] 继续扩展 GFM 覆盖。
- [ ] 强化性能 — 结构化 1–100 MB parser 阶段现已接近线性增长；降低 cmark/AST 构建内存与解析后实体化成本。
